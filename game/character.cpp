...
namespace Fly
{
  ...
  namespace MUD
  {
    ...
    Equipment::Equipment(Type type, Quality quality, ::std::string name, uint32_t atk, uint32_t hp, uint32_t level)
    {
      this->type = type;
	...
	//return c->get_atk(true) + (c->level) + equipment_atk;
	return c->get_atk(true) + equipment_atk;
      };
	...
	//return c->get_hp(true) + (c->level) + equipment_hp;
	return c->get_hp(true) + equipment_hp;
      };
    }
